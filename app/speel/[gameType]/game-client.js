...
export default function GameClient({
  ...
  const isGameOver =
    currentGuesses.length === boardSize || isVictory;
  ...
  const submit = useCallback(
    async (text) => {
      ...
      // Hard mode validation - must use all previously found letters
      ...
      // Get new random word if user guessed the demo word
      if (text === randomWord) {
        // Just clear it for now, we don't need to fetch a new one immediately
        setRandomWord("");
      }

      try {
        // Use server action instead of GraphQL
